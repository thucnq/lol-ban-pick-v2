...
func createSupportLookup(itemsets []itemsetWithCount, numMatches int) *itemsetSupportLookup {
	isl := newItemsetSupportLookup()
	f := float64(numMatches)
...
func generateRules(itemsets []itemsetWithCount, numMatches int, minConfidence float64, minLift float64) [][]Rule {
	...
	itemsetSupport := createSupportLookup(itemsets, numMatches)

	lastFeedback := time.Now()

	for index, itemset := range itemsets {
		support := float64(itemset.count) / float64(numMatches)
